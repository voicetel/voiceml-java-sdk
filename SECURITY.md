...
<https://github.com/voicetel/voiceml-java-sdk/security/advisories/new>).
...
- Do not log the configured API key / auth token or the
  `Authorization` header — both carry the per-tenant secret in HTTP
  Basic form.
- Keep the account SID + secret out of source control; load from a
  secret manager or environment.
- The SDK uses `java.net.http.HttpClient` with the JDK default TLS
  posture (TLS 1.2+ via JSSE). If you supply a custom builder, you
...
Out of scope: vulnerabilities in the published `com.voicetel:voiceml`
Maven artifact caused by a forked / vendored copy that has been
modified.
